...
class RocksDBSSTWriter implements AutoCloseable {
	...
	/** {@code DEFAULT_OPTIONS} provides the default RocksDB {@link Options} for {@link SstFileWriter}. It is a singleton so it does not need to be closed. */
	private static final Options DEFAULT_OPTIONS = new Options();

	/** {@code DEFAULT_ENV_OPTIONS} provides the default RocksDB {@link EnvOptions} for {@link SstFileWriter}. It is a singleton so it does not need to be closed. */
	...
	public RocksDBSSTWriter(
		...
		if (file.exists()) {
			throw new IOException(
				"File provided for sst " + "writing \"" + file.getAbsolutePath() + "\" already exists.");
		}
	...
	public void put(@Nonnull byte[] key, @Nonnull byte[] value) throws RocksDBException {
		Preconditions.checkState(
			!finished, "attempted to `put` on RocksDBSSTWriter after calling `finish` (or closing)");
