...
/**
 * {@link RocksDBSSTIngestWriter} implements {@link RocksDBWriter}, providing
 * writes by creating sst files and instructing {@link RocksDB} to ingest them
 * (via {@link
 * RocksDB#ingestExternalFile(ColumnFamilyHandle, List, IngestExternalFileOptions)}).
 * It supports writing to multiple {@link ColumnFamilyHandle}, assuming the
 * writes within a {@link ColumnFamilyHandle} are ordered.
 *
 ...
 *
 * <p>{@link RocksDBSSTIngestWriter} is for batch write operations. It produces
 * and ingests individual files throughout put operations, so it will not
 * atomically commit data.
 */
public class RocksDBSSTIngestWriter implements RocksDBWriter {
	...
	private static final IngestExternalFileOptions INGEST_EXTERNAL_FILE_OPTIONS =
		new IngestExternalFileOptions(
			...
			// import fails.
			//
			// @lgo: fixme ensure this does not cause stray RocksDB sst files on a host when ingestion
			// fails.
			/* moveFiles */ true,
	...
	/**
	 * {@code sstFileSizes} contains the current byte-sizes for the currently opened {@link RocksDBSSTWriter}, for each {@link ColumnFamilyHandle}.
	 */
	...
	private void flushIfNeeded(ColumnFamilyHandle handle, RocksDBSSTWriter writer)
		...
		//   every 1mb. https://github.com/facebook/rocksdb/blob/096beb787eed5837a2e38a9681ee834e9268c881/table/sst_file_writer.cc#L102
		// - the underlying block builder will flush based on the policy RocksDB
		//   was set up with: https://github.com/facebook/rocksdb/blob/096beb787eed5837a2e38a9681ee834e9268c881/table/block_based/block_based_table_builder.cc#L773
		// - based on table_options.block_size (and block_size_deviation), the FS automatically flushes.
		//
