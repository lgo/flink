	...
	public void basicTest() throws Exception {
		...
		for (int i = 0; i < 10000; ++i) {
			firstColumnFamilyData.add(
				new Tuple2<>(
					...
					(String.format("family-1-key:%010d", i)).getBytes(), ("value:" + i).getBytes()));
		}
		...
		for (int i = 0; i < 10000; ++i) {
			secondColumnFamilyData.add(
				new Tuple2<>(
					// Pad the key integer so data is in ascending key-order.
					(String.format("family-2-key:%010d", i)).getBytes(), ("value:" + i).getBytes()));
		}
		...
		try (RocksDB db = RocksDB.open(folder.newFolder().getAbsolutePath());
			...
			ColumnFamilyHandle firstHandle =
				db.createColumnFamily(new ColumnFamilyDescriptor("test-handle-1".getBytes()));
			ColumnFamilyHandle secondHandle =
				db.createColumnFamily(new ColumnFamilyDescriptor("test-handle-2".getBytes()));
			RocksDBWriter writer =
				new RocksDBSSTIngestWriter(
					db, 200, null, null, folder.newFolder())) {
