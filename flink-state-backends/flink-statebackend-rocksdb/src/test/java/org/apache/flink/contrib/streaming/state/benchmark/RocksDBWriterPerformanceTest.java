...
public class RocksDBWriterPerformanceTest extends TestLogger {
	...
	public void benchMark() throws Exception {
		...
		for (int i = 0; i < num; ++i) {
			data.add(new Tuple2<>(String.format("%s-%08X", KEY_PREFIX, i).getBytes(), VALUE.getBytes()));
		}
	...
	private enum WRITETYPE {PUT, WRITE_BATCH, SST_INGEST}
		...
		switch (type) {
			...
			case SST_INGEST:
				try (RocksDB db = RocksDB.open(rocksDir.getAbsolutePath());
					...
					ColumnFamilyHandle handle = db.createColumnFamily(new ColumnFamilyDescriptor("test".getBytes()));
					RocksDBWriter writer = new RocksDBSSTIngestWriter(db,
						MemorySize.parse("16mb").getBytes(), null, null, folder.newFolder())) {
					long t1 = System.nanoTime();
					for (Tuple2<byte[], byte[]> item : data) {
						writer.put(handle, item.f0, item.f1);
					}
